...
export class GoogleStrategy<User> extends OAuth2Strategy<User> {
  ...
  constructor(
    ...
    verify: (opts: { profile: GoogleProfile }) => Promise<User>,
  ) {
    super(
      {
...
function parseScope(scope: GoogleStrategyOptions['scope']): string[] {
  if (!scope) {
  ...
  return scope.split(GoogleStrategyScopeSeperator).map((s) => s.trim());
}
